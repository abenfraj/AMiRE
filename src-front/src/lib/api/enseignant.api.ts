...
export class EnseignantApi {
     ...
     * PATCH /teacher/:id
     */
    public static async saveEnseignant(teacher: Partial<EnseignantEntity>): Promise<EnseignantEntity> {
    ...
    public static async getCandidatures(): Promise<CandidatureEntity[]> {
        ...
        await tmpPause();
        return __tmpCandidatures.filter((c) => c.idEnseignant === __tmpTeacher.idEnseignant);
    }
};
