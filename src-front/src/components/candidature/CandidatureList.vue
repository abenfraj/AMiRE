<template>
    <table class="table">
        ...
        <tbody>
            <CandidatureItem v-for="candidature in props.candidatures" :candidature="candidature" @openProfil="handleOpenProfil" @candidature-status-update="$emit('candidature-status-update')"></CandidatureItem>
        </tbody>
        <ModalVue title="Voir un profil" :isLoading="false" :isActive="openedProfile >= 0" variant="ok" @close="handleCloseProfil">
            <UserProfileContainer :userId="openedProfile" v-if="openedProfile >= 0"></UserProfileContainer>
        </ModalVue>
    </table>
</template>
