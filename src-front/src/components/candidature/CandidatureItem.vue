<template>
    <tr v-if="props.candidature" class="is-relative">
        ...
        <td>
            <button v-if="account.type.value == AccountType.Ecole" class="button is-text is-small" @click="handleVoirProfil">Voir le profil du candidat</button> 
            <div v-else>
                <p>{{props.candidature.annonce?.titre}}</p>

                <p class="subtitle is-7">{{props.candidature.annonce?.description}}</p>
            </div>
        </td>
        ...
        <td v-if="props.candidature.decision != 'Attente'">
            ...
            <em class="is-size-7">
                par {{props.candidature.contacteParPersonne}} le {{props.candidature.contacteLe}}
            </em>
        </td>
        <td v-else>
            <p v-if="account.type.value == AccountType.Enseignant" class="tag">{{props.candidature.decision}}</p>
            <div v-else class="buttons">
                <button class="button is-small " @click="changeDecision('Refusée')">Refuser</button>
                <button class="button is-small is-success" @click="changeDecision('Acceptée')">Valider</button>
            </div>
        </td>
...
async function changeDecision(decision: "Acceptée" | "Refusée") {
    ...
    const candidature: Partial<CandidatureEntity> = { ...props.candidature, decision, contacteLe, contacteParPersonne };
